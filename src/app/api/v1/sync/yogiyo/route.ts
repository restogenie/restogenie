    ...
    try {
        ...
        if (body.store_id) {
            ...
            const connection = await prisma.posConnection.findFirst({
                where: { store_id: storeId, vendor: "yogiyo", is_active: true }
            });
            if (!connection || (!connection.auth_code_1 || !connection.auth_code_2)) {
                return NextResponse.json({ detail: "Active Yogiyo connection (ID/PW) not found for this store" }, { status: 404 });
            }
        ...
        if (storesToSync.length > 0) {
            ...
            const scriptPath = [process.cwd(), "scripts", "yogiyo_crawler.js"].join("/");
            console.log(`[Yogiyo Background Sync Scheduled] Store ID: ${storeId}`);
            ...
            return NextResponse.json({
                status: "success",
                message: "Yogiyo sync task has been scheduled in the background.",
        ...
        return NextResponse.json({
            status: "success",
            message: `Yogiyo global sync triggered.`,
            data: []
        });
    } catch (error: any) {
        console.error("Yogiyo Sync API Error:", error);
        await sendSystemAlert(`Yogiyo Global Sync API Error`, error.message);
        return NextResponse.json(
            { status: "error", message: error.message || "Failed to trigger Yogiyo sync." },
            { status: 500 }
        );
    }
}
